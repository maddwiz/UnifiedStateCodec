...
MAGIC = b"USCDO1"  # TemplateMTF(BitPack IDs) + DeltaOnly Values v0.1
...
def encode_chunks_with_template_mtf_bits_deltaonly(chunks: List[str]) -> bytes:
    """
    TMTFDO v0.1
    ✅ IDs = MTF + bitpack
    ✅ Values:
       - first time a template-id appears => store ABS values
       - afterwards => store only zigzag(delta from last seen values)
    """
    templates: List[str] = []
...
def decode_chunks_with_template_mtf_bits_deltaonly(packet_bytes: bytes) -> List[str]:
    raw = gzip.decompress(packet_bytes)
    if not raw.startswith(MAGIC):
        raise ValueError("Not a TMTFDO packet")
