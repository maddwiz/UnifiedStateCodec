...
MAGIC = b"USSWCZ1"  # USC Stream Window CANZ v1
...
def encode_stream_window_canz(chunks: List[str], state: StreamState | None = None) -> bytes:
    """
    Stream Window CANZ:
    ...
    new_templates: List[Tuple[int, str]] = []

    # Extract templates + values, and update template dictionary if new
    for ch in chunks:
    ...
    # values (delta-only across stream per template)
    for tid, vals in zip(tids, values_per_chunk):
        ...
        if not state.seen_tid.get(tid, False):
            ...
            prev = state.prev_vals_by_tid.get(tid, [0] * len(vals))
            new_prev: List[int] = []
...
def decode_stream_window_canz(packet_bytes: bytes, state: StreamState | None = None) -> List[str]:
    """
    ...
    if not raw.startswith(MAGIC):
        raise ValueError("Not a Stream Window CANZ packet")
    ...
    for tid in tids:
        ...
        if not state.seen_tid.get(tid, False):
            ...
            prev = state.prev_vals_by_tid.get(tid, [0] * nvals)
            new_prev: List[int] = []
