...
# v4 tokenized dict+data
from usc.mem.stream_proto_canz_v4 import (
    StreamStateV4,
    build_dict_state_from_chunks as build_v4,
    encode_dict_packet_tokenized as dict_v4,
    apply_dict_packet_tokenized as apply_v4,
    encode_data_packet as data_v4,
)
...
def run():
    ...
    total3_first = len(pkt3_dict) + len(pkt3_data)

    # ---- v4 ----
    st4_build = StreamStateV4()
    build_v4(chunks, state=st4_build)
    pkt4_dict = dict_v4(st4_build, level=10)

    st4_send = StreamStateV4()
    apply_v4(pkt4_dict, state=st4_send)
    pkt4_data = data_v4(chunks, st4_send, level=10)

    total4_first = len(pkt4_dict) + len(pkt4_data)

    print("USC Stream Bench v6 — DICT Shrink (v3 vs v4)")
    print("-------------------------------------------------")
    ...
    print(f"v3 FIRST total  : {total3_first}")
    print("-------------------------------------------------")
    print(f"v4 DICT bytes   : {len(pkt4_dict)}")
    print(f"v4 DATA bytes   : {len(pkt4_data)}")
    print(f"v4 FIRST total  : {total4_first}")
    print("-------------------------------------------------")
    print("✅ Goal: v4 FIRST total < v3 FIRST total")
    print("✅ Stretch: v4 FIRST total < GZIP")
    print("-------------------------------------------------")
