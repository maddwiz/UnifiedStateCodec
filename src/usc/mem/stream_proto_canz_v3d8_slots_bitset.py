...
def _slot_structs(state: StreamStateV3D8, tid: int, slot: int) -> Tuple[List[str], Dict[str, int], List[int]]:
    key = (tid, slot)
...
# dict packet
# -------------------------
def build_dict_state_from_chunks(chunks: List[str], state: StreamStateV3D8) -> None:
...
def encode_dict_packet(state: StreamStateV3D8, level: int = 10) -> bytes:
    out = bytearray()
    out += MAGIC_DICT
...
def apply_dict_packet(packet: bytes, state: StreamStateV3D8) -> None:
...
# -------------------------
# data packet (bitset param types)
# -------------------------
def encode_data_packet(chunks: List[str], state: StreamStateV3D8, level: int = 10) -> bytes:
    if state.miner is None:
        state.miner = _make_miner()
    ...
    for tid, tmpl_mined, params in zip(tids_for_chunks, mined_templates, mined_params):
        ...
        for slot_i, p in enumerate(params):
            ...
            if _looks_int(p):
                ...
                num_stream += encode_uvarint(_zigzag_encode(delta))
            else:
                type_flags.append(1)
    ...
    # numeric deltas stream
    out += encode_uvarint(len(num_stream))
    out += bytes(num_stream)

    # string positions stream
    out += encode_uvarint(len(str_stream))
    out += bytes(str_stream)

    return zstd_compress(bytes(out), level=level)


def decode_data_packet(packet: bytes, state: StreamStateV3D8) -> List[str]:
    raw = zstd_decompress(packet)
    if not raw.startswith(MAGIC_DATA):
        raise ValueError("not a v3d8 DATA packet")
    ...
    # numeric stream
    num_len, off = decode_uvarint(raw, off)
    num_bytes = raw[off:off + num_len]
    off += num_len

    # string stream
    str_len, off = decode_uvarint(raw, off)
    str_bytes = raw[off:off + str_len]
    off += str_len

    # reconstruct by scanning flags
    num_off = 0
    str_off = 0
    ...
    for i in range(n_chunks):
        ...
        for slot_i in range(arity):
            ...
            if is_str:
                pos, str_off = decode_uvarint(str_bytes, str_off)
                slist, sindex, smtf = _slot_structs(state, tid, slot_i)
                sid = _mtf_get_and_move(smtf, pos)
                vals.append(slist[sid])
            else:
                z, num_off = decode_uvarint(num_bytes, num_off)
